...
def GetRFData(dataPath,opt,use_embedding):
    ...
    if use_embedding:
        ...
        rf_embedding_path = dataPath.replace("rf_data","rf_embedding") + ".npy"

        rf_data = np.load(rf_embedding_path)
...
class MyDataSet(Dataset):
    def __init__(self,root,opt,img_transform=None, rf_transform=None,mode="train",use_input="rf_data",use_embedding = False):

        self.ct_dir = os.path.join(root, "%s/ct/" % mode)
        self.rf_dir = os.path.join(root, "%s/rf_data/" % mode)
        self.us_dir = os.path.join(root, "%s/us/" % mode)
        ...
        self.us_list = os.listdir(self.us_dir)
        self.ct_list = os.listdir(self.ct_dir)
    ...
    def __getitem__(self, idx):
        ...
        if self.use_input == "rf_data" or self.use_input == "all":
            rf_name = us_name.replace(".png","")
            rf_path = os.path.join(self.rf_dir, rf_name)

            rf_data = GetRFData(rf_path, self.opt, self.use_embedding)

            if self.rf_transform:
                rf_data = self.rf_transform(rf_data)

        if self.use_input == "ct" or self.use_input == "all":
            ct_name = us_name
        ...
        if self.use_input == "ct":
            return {"ct": ct, "us": us,"name":us_name.replace(".png","")}
        elif self.use_input == "rf_data":
            return {"rf_data": rf_data, "us": us,"name":us_name.replace(".png","")}
        else:
            return {"ct": ct,"rf_data": rf_data, "us": us,"name":us_name.replace(".png","")}
