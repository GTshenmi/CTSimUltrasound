...
class MyDataSet(Dataset):
    def __init__(self,root,args,img_transform=None, rf_transform=None,mode="train",use_input="rf_data",use_embedding = False):
        ...
        self.rf_transforms = transforms.Compose([
            transforms.ToTensor(),
        ])
        ...
        if img_transform:
            self.img_transforms = transforms.Compose(img_transform)
        if rf_transform:
            self.rf_transforms = transforms.Compose(rf_transform)
    ...
    def __getitem__(self, idx):
        ...
        if self.img_transforms:
            us = self.img_transforms(us)

        rf_name = us_name.replace(".png","")
        rf_path = os.path.join(self.rf_dir, rf_name)

        rf_data = GetRFData(rf_path, self.args)

        if self.rf_transforms:
            rf_data = self.rf_transforms(rf_data)
