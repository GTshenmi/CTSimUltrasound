...
class AutoEncoder(nn.Module):
    def __init__(self,device):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(64,1, 3, stride=1, padding=1),  # b, 4, 256, 1024
            nn.InstanceNorm2d(1),
            nn.LeakyReLU(0.2, inplace=True),
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ConvTranspose2d(1, 64,(3,4), stride=(1,4), padding=(1,0)),  # b, 64, 256, 1024
            nn.LeakyReLU(0.2, inplace=True),
    ...
    def training_losses(self,x):

        enc,dec = self.forward(x)
        loss = self.criterion(dec,x)
